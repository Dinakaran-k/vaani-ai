...
Vaani AI is a Flutter and Firebase based mobile app foundation for small businesses in India. It helps merchants manage inventory, sales, invoice scanning, and payment reminders through voice-first and mobile-first workflows.
...
Small merchants often manage business operations through paper notes, spreadsheets, WhatsApp messages, and separate apps. These workflows are difficult to search, reconcile, and scale. Vaani AI provides one assistant-driven interface for common daily operations while keeping the merchant in control of important data changes.
...
- Keep the app useful during poor connectivity through local queues and later sync.
...
- Sales domain model and sales screen shell.
...
- Firebase Auth, Cloud Firestore, Firebase Storage, Messaging, Analytics, and Crashlytics.
...
- Gemini and OpenAI access planned through a secure Cloud Functions gateway.
...
- AI providers must be called through backend functions, not directly from the client with secrets.
...
Vaani AI uses AI for natural language interpretation, not uncontrolled data mutation. Common commands should be handled through deterministic shortcuts first. Unmatched commands may be routed through Gemini or OpenAI via Cloud Functions. The returned intent must match a typed schema before the app performs any business operation.
...
- Add Cloud Functions gateway for AI, WhatsApp, SMS, email, and provider secrets.
